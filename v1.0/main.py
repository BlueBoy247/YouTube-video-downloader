...
def clickDown():
    global getvideo,strftype,listradio,path,IsChecking
    labelMsg.config(text="")
    if IsChecking==False:
    ...
    if path.get()=="":
        ...
        #labelPAName.config(text=path.get())
    try:
        yt=YouTube(url.get())
        if DLname.get()=="":
            Fname=yt.title
...
mainw.resizable(0,0)
mainw.title("Shuba_YT影片下載器 v1.0")
